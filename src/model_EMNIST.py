...
class FusionNet(nn.Module):
    """A fusion model that adds a terminal fully connected layer to linearly combine
       score functions already parametrized by the UNet-based architectures.
    """
    def __init__(self, comp_scores: nn.ModuleList, lambdas = None):
        ...
        self.score_count = len(comp_scores)
        self.manual = False

        for m in comp_scores: # freeze the model parameter gradients
            for param in m.parameters():
                param.requires_grad = False

        # linear combo weights (un-normalized)
        if lambdas is None:
            self.lambdas_logits = nn.Parameter(torch.randn(self.score_count))
        else:
            self.lambdas_logits = lambdas
            self.manual = True
    ...
    def forward(self, x, t):
        ...
        '''
        # Code below pending completion
        hs = torch.zeros(self.score_count, x.shape[0], x.shape[1], x.shape[2],
                         x.shape[3], device = device) # (batch_size, score_count)

        for i, s in enumerate(self.comp_scores):
            hs[i,...] = s(x, t) # compute the vector value of each component score

        if self.manual:
            weights = self.lambdas_logits.view(self.score_count, 1, 1, 1, 1)
        else:
            weights = F.softmax(self.lambdas_logits, dim = 0).view(self.score_count, 1, 1, 1, 1)
        # weights = self.lambdas_logits.view(self.score_count, 1, 1, 1, 1)

        return torch.sum(weights * hs, dim = 0)
...
def fusion_loss_fn(model, x, marginal_prob_std, eps=1e-3):
  """The loss function for training the fusion method.
