...
lambdas_info = [ # entered from fusion training results
   {'train_size': 64, 'weights': [0.199, 0.305, 0.279, 0.217]},
   {'train_size': 128, 'weights': [0.187, 0.326, 0.267, 0.22]},
   {'train_size': 256, 'weights': [0.182, 0.328, 0.284, 0.206]},
   {'train_size': 512, 'weights': [0.181, 0.319, 0.285, 0.216]},
   {'train_size': 1024, 'weights': [0.167, 0.345, 0.319, 0.170]},
   {'train_size': 4096, 'weights': [0.183, 0.311, 0.294, 0.213]},
   {'train_size': 16384, 'weights': [0.176, 0.310, 0.295, 0.220]}
]

NUM_REPEATS = 2 # CHANGEABLE PARAMETERS OF THIS FILE!!! - how many NLL evals to run
...
target_info = {
    ...
    'test_size': 5000,'test_batch': 500 # CHANGEABLE PARAMETERS OF THIS FILE!!!
}
...
for i, info in enumerate(lambdas_info):
    # For each auxiliary model, compute its NLL on the target test set
    train_size, weights = info['train_size'], info['weights']
    print(f'-----Fusion model Test NLL, trained on {train_size} data-----')
    print(f'Weights: {weights}')
    weights = torch.tensor(weights, device=device) # convert to tensor
    fusion_score = torch.nn.DataParallel(FusionNet(aux_scores, weights))
    nll_estimators = []
    for _ in range(NUM_REPEATS):
      for x in test_tar_loader:
          ...
          _, fusion_bpd = ode_likelihood(x, fusion_score, marginal_prob_std_fn,
                                  diffusion_coeff_fn,
                                  x.shape[0], device=device, eps=1e-5)        
          fusion_estimator = fusion_bpd.sum() / fusion_bpd.shape[0]
          nll_estimators.append(fusion_estimator)
          x.to('cpu') # release GPU memory
          # print(f'Elapsed time per one batch ODE eval: {(time.time() - start_time)/60}')
    nll_estimators = torch.tensor(nll_estimators, device = 'cpu')
    print(f'NLL mean: {nll_estimators.mean()}, std: {nll_estimators.std()}\n')