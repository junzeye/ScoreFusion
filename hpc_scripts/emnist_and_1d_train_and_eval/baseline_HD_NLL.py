...
base_scores = nn.ModuleList() # store the score models
path_infos = [
   {'train_size': 64, 'val_size': 16},
   {'train_size': 128, 'val_size': 32},
   {'train_size': 256, 'val_size': 64},
   {'train_size': 512, 'val_size': 256},
   {'train_size': 1024, 'val_size': 256},
   {'train_size': 4096, 'val_size': 1024},
   {'train_size': 16384, 'val_size': 4096},
]
for path_info in path_infos: # load the base models
    train_size, val_size = path_info['train_size'], path_info['val_size']
    ckpt = torch.load(f'ckpt/s10/base_hd_{train_size}_{val_size}.pth', map_location=device)
    score = torch.nn.DataParallel(ScoreNet(marginal_prob_std=marginal_prob_std_fn))
...
for i, (path_info, score) in enumerate(zip(path_infos, base_scores)):
    # For each auxiliary model, compute its NLL on the target test set
    train_size, val_size = path_info['train_size'], path_info['val_size']
    print(f'-----Base model train={train_size}, val={val_size}, Test NLL-----')
    nll_estimators = []
    for _ in range(NUM_REPEATS):
      for x in test_tar_loader:
          ...
          _, fusion_bpd = ode_likelihood(x, score, marginal_prob_std_fn,
                                  diffusion_coeff_fn,
